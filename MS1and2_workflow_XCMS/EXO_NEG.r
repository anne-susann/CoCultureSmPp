...
all_files_names[26:(25 + length(raw_data_MS2_exo_neg))]<- str_remove_all(all_files_names[26:(25 + length(raw_data_MS2_exo_neg))], input_dir_MS2)
all_files_names
...
if (dir.exists(paste(getwd(), "/exo_neg_1ms2_Results/", sep = ""))){
  print("plots directory already exists")
  start_time <- Sys.time()
}  else{
  dir.create("exo_neg_1ms2_Results")
  start_time <- Sys.time()
  print("results folder has been created")
}
write.csv(fData(msd), file=paste(filename = "exo_neg_1ms2_Results/EXO_neg_raw_data.csv", sep = ""), row.names=FALSE)
...
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_chromas.jpeg", width = 2000, height = 1200, quality = 100, bg = "white")
...
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_tics.jpeg", width = 2000, height = 1200, quality = 100, bg = "white")
par(mfrow=c(1,1), mar=c(5,5,4,1), oma=c(0,0,0,0), cex.axis=1.5, cex=2, cex.lab=2, cex.main=2)
...
jpeg(filename = "exo_neg_1ms2_plots/heatmap_chromas_bin_exo_neg.jpeg", width = 1000, height = 1000, quality = 100, bg = "white")
...
write.csv(as.data.frame(table(msLevel(ms_data_exo_neg))), file="exo_neg_1ms2_Results/EXO_neg_ms_data.csv", row.names=FALSE)

jpeg(filename = "exo_neg_1ms2_plots/exo_neg_ms_data.jpeg", width = 2000, height = 1200, quality = 100, bg = "white")
...
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_ms_raw_adjusted.jpeg", width = 500, height = 1000, quality = 100, bg = "white")
par(mfrow=c(2,1), mar=c(4.5,4.2,4,1), cex=0.8)
plot(chromas_exo_neg, peakType="none", main="Raw chromatograms")
plotAdjustedRtime(filterRt(ms_data_exo_neg, rt = c(0, 670)), lwd=2, main="Retention Time correction")
par(mfrow=c(1,1), mar=c(4,4,4,1), oma=c(0,0,0,0), cex.axis=0.9, cex=0.8)
text(labels=ms_data_exo_neg$sample_name, pos=3, cex=1)
dev.off()
...
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_ms_grouping.jpeg", width = 1000, height = 700, quality = 150, bg = "white")
...
plot(ms_pca_exo_neg$x[, 1], ms_pca_exo_neg$x[,2], pch=19, main="PCA: Grouping of samples",
     ...
     col=color, cex=2)
grid()
text(ms_pca_exo_neg$x[, 1], ms_pca_exo_neg$x[,2], labels=str_sub(ms_data_exo_neg$sample_name, - 3, - 1), col=color, pos=3, cex=0.9)
...
# broken stick
png("exo_neg_1ms2_plots/BrokenStick_exo_neg_ms_grouping.png", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
...
save(MS_exo_neg_peak_detection, file = "exo_neg_1ms2_Results/MS_exo_neg_peak_detection.RData")
...
# save as csv
write.csv(feat_list_exo_neg, file=paste(filename = "exo_neg_1ms2_Results/feature_list_exo_neg.csv", sep = ""))

# Plot histogram
#pdf(file="plots/exo_feat_list_hist.pdf", encoding="ISOLatin1", pointsize=10, width=6, height=4, family="Helvetica")
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_feat_list_hist.jpeg", width = 500, height = 500, quality = 150, bg = "white")
...
jpeg(filename = "exo_neg_1ms2_plots/exo_neg_ms_feature_table_pca.jpeg", width = 1000, height = 700, quality = 150, bg = "white")
...
# broken stick
jpeg("exo_neg_1ms2_plots/BrokenStick_exo_neg_ms_feature_table_pca.jpeg", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
  # Broken stick model (MacArthur 1957)  
  n = length(ev)  
  bsm = data.frame(j=seq(1:n), p=0)  
...
# save as csv
write.csv(bina_list_exo_neg, file=paste(filename = "exo_neg_1ms2_Results/bina_list_exo_neg.csv", sep = ""))
...
# funtional hill
#model_div_pos$hillfunc    <- as.numeric(unlist(calcDiv(feat_list_exo_neg, compDisMat=scales::rescale(as.matrix(dist(t(feat_list_exo_neg)), diag=TRUE, upper=TRUE)), q=1, type="FuncHillDiv")))

# Remove NAs if present
model_div_exo_neg[is.na(model_div_exo_neg)] <- 0

# save as csv
write.csv(model_div_exo_neg, file=paste(filename = "exo_neg_1ms2_Results/model_div_exo_neg.csv", sep = ""))


# save the objects and tables
save(ms_def_exo_neg, file = "exo_neg_1ms2_Results/ms_def_exo_neg.RData")
save.image(file = "exo_neg_1ms2_Results/exo_neg_ms_environment.RData")
