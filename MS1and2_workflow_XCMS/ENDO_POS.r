...
# sps_all <- Spectra(paste(input_dir_MS1, "/", raw_data_MS1_ENDO_pos, sep = ""), source = MsBackendMzR())
# sps_all

raw_data_MS1_ENDO_pos <- list.files(input_dir_MS1, pattern = "pos_ENDO")
raw_data_MS1_ENDO_pos_files <- paste(input_dir_MS1, "/", raw_data_MS1_ENDO_pos, sep ="")
length(raw_data_MS1_ENDO_pos_files)
...
all_files_names[28]<- str_remove_all(all_files_names[28], '/Users/mahnoorzulfiqar/OneDriveUNI/GitHub-Repos/CoCulture2/MS2/')

all_files_names[29]<- str_remove_all(all_files_names[29], '/Users/mahnoorzulfiqar/OneDriveUNI/GitHub-Repos/CoCulture2/MS2/')

all_files_names[30]<- str_remove_all(all_files_names[30], '/Users/mahnoorzulfiqar/OneDriveUNI/GitHub-Repos/CoCulture2/MS2/')
...
table(msLevel(msd))
write.csv(fData(msd), file=paste(filename = "Results/ENDO_pos_raw_data.csv", sep = ""), row.names=FALSE)
...
jpeg(filename = "plots/ENDO_chromas_pos.jpeg", width = 1000, height = 600, quality = 100, bg = "white")
...
jpeg(filename = "plots/ENDO_tics_pos.jpeg", width = 1000, height = 600, quality = 100, bg = "white")
...
jpeg(filename = "plots/heatmap_chromas_bin_ENDO_pos.jpeg", width = 500, height = 500, quality = 100, bg = "white")
par(mfrow=c(1,1), mar=c(4,4,4,1), oma=c(0,0,0,0), cex.axis=0.9, cex=0.6)
...
write.csv(as.data.frame(table(msLevel(ms_data_ENDO_pos))), file="Results/ENDO_pos_ms_data.csv", row.names=FALSE)
...
plotAdjustedRtime(ms_data_ENDO_pos, lwd=2, main="Retention Time correction")
...
jpeg(filename = "plots/ENDO_pos_ms_grouping.jpeg", width = 1000, height = 500, quality = 150, bg = "white")
...
plot(ms_pca_ENDO_pos$x[, 1], ms_pca_ENDO_pos$x[,2], pch=19, main="PCA: Grouping of samples",
     ...
     col=color, cex=0.8)
grid()
text(ms_pca_ENDO_pos$x[, 1], ms_pca_ENDO_pos$x[,2], labels=ms_data_ENDO_pos$sample_name, col=color, pos=3, cex=0.9)
...
# broken stick
png("plots/BrokenStick_ENDO_pos_ms_grouping.png", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
...
save(MS_ENDO_pos_peak_detection, file = "Results/MS_ENDO_pos_peak_detection.RData")
...
# save as csv
write.csv(feat_list_ENDO_pos, file=paste(filename = "Results/feature_list_ENDO_pos.csv", sep = ""))

# Plot histogram
#pdf(file="plots/ENDO_feat_list_hist.pdf", encoding="ISOLatin1", pointsize=10, width=6, height=4, family="Helvetica")
jpeg(filename = "plots/ENDO_pos_feat_list_hist.jpeg", width = 500, height = 500, quality = 150, bg = "white")
...
jpeg(filename = "plots/ENDO_pos_ms_feature_table_pca.jpeg", width = 1000, height = 500, quality = 150, bg = "white")
...
# broken stick
jpeg("plots/BrokenStick_ENDO_pos_ms_feature_table_pca.jpeg", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
  # Broken stick model (MacArthur 1957)  
  n = length(ev)  
  bsm = data.frame(j=seq(1:n), p=0)  
...
# save as csv
write.csv(bina_list_ENDO_pos, file=paste(filename = "Results/bina_list_ENDO_pos.csv", sep = ""))
...
# save as csv
write.csv(model_div_ENDO_pos, file=paste(filename = "Results/model_div_ENDO_pos.csv", sep = ""))
