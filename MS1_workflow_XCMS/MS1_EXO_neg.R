...
# for EXO_neg files, create list of MS1 files
...
#EXO_neg files
MS1_EXO_neg_files <- list.files(input_dir_MS1_polarity, pattern = "_neg_EXO")
# Import raw data as MSnbase object OnDiskMsnExp


# only MS1 data
msd <- readMSData(files = paste(input_dir_MS1_polarity, MS1_EXO_neg_files, sep = ""),
...
#ms1_data_EXO_neg <- NULL 
#condition_name <- deparse(substitute(ms1_data_EXO_neg))
#condition_name
...
# Per file summary
ms1_summary_EXO_neg <- lapply(split.data.frame(chromPeaks(ms1_data_EXO_neg), 
                                            f=chromPeaks(ms1_data_EXO_neg)[, "sample"]), 
                           FUN=function(z) { c(peak_count=nrow(z), rt=quantile(z[, "rtmax"] - z[, "rtmin"])) } )
ms1_summary_EXO_neg <- do.call(rbind, ms1_summary_EXO_neg)
rownames(ms1_summary_EXO_neg) <- basename(fileNames(ms1_data_EXO_neg))
print(ms1_summary_EXO_neg)
table(msLevel(ms1_data_EXO_neg))

write.csv(as.data.frame(table(msLevel(ms1_data_EXO_neg))), file="exo_neg_Results/EXO_neg_ms1_data.csv", row.names=FALSE)
...
## Group peaks
ms1_data_EXO_neg <- groupChromPeaks(ms1_data_EXO_neg, param=PeakDensityParam(
  sampleGroups=ms1_data_EXO_neg$sample_group, minFraction=0.7, bw=2.5))

## RT correction
ms1_data_EXO_neg <- adjustRtime(ms1_data_EXO_neg, param=PeakGroupsParam(
...
jpeg(filename = "exo_neg_plots/EXO_neg_ms1_raw_adjusted.jpeg", width = 1000, height = 2000, quality = 100, bg = "white")
...
plotAdjustedRtime(ms1_data_EXO_neg, lwd=2, main="Retention Time correction")
...
print(head(featureValues(ms1_data_EXO_neg, value="into")))
...
plot(ms1_pca_EXO_neg$x[, 1], ms1_pca_EXO_neg$x[,2], pch=19, main="PCA: Grouping of samples",
     ...
     col=color, cex=2)
grid()
text(ms1_pca_EXO_neg$x[, 1], ms1_pca_EXO_neg$x[,2], labels=str_sub(ms1_data_EXO_neg$sample_name, - 3, - 1), col=color, pos=3, cex=1.5)
...
# broken stick
png("exo_neg_plots/BrokenStick_EXO_neg_ms1_grouping.png", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
...
ms1_matrix_EXO_neg <- featureValues(ms1_data_EXO_neg, method="medret", value="into")
colnames(ms1_matrix_EXO_neg) <- MS1_EXO_names
dim(ms1_matrix_EXO_neg)
# transpose feature table
feat_list_EXO_neg <- t(ms1_matrix_EXO_neg)

# Build feature summary
ms1_summary_EXO_neg <- featureSummary(ms1_data_EXO_neg)
ms1_def_EXO_neg <- featureDefinitions(ms1_data_EXO_neg)
...
feat_list_EXO_neg[which(feat_list_EXO_neg == 0)] <- 0.01

# Missing value imputation
feat_list_EXO_neg[which(is.na(feat_list_EXO_neg))] <- 0
...
plot(ms1_pca_EXO_neg$x[, 1], ms1_pca_EXO_neg$x[,2], pch=19, main="PCA of feature table",
     ...
     col=color, cex=2)
grid()
text(ms1_pca_EXO_neg$x[,1], ms1_pca_EXO_neg$x[,2], labels=str_sub(ms1_data_EXO_neg$sample_name[1:24], - 3, - 1), col=color, pos=3, cex=1.5)
legend("topleft", bty="n", pt.cex=2, cex=2, y.intersp=0.7, text.width=0.5, pch=20, 
       col= unique(color), legend= unique(ms1_data_EXO_neg@phenoData@data[["sample_group"]]))
dev.off()

# broken stick
png("exo_neg_plots/BrokenStick_EXO_neg_ms1_feature_table_pca_exc_MB.png", width=10, height=6, units="in", res=100)
evplot = function(ev) {  
  # Broken stick model (MacArthur 1957)  
  n = length(ev)  
  bsm = data.frame(j=seq(1:n), p=0)  
...
feature_info_EXO_neg <- data.frame(cbind(ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@rownames, 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["mzmed"]], 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["mzmin"]], 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["mzmax"]], 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["rtmed"]], 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["rtmin"]], 
                                          ms1_data_EXO_neg@msFeatureData[["featureDefinitions"]]@listData[["rtmax"]]))
# add column names
...
for (i in 1:nrow(ms1_def_EXO_neg)) {
  ...
  if (length(which(ms2_data_EXO_neg$peak_id %in% rownames(peaks_of_interest))) > 0) {
    # Extract spectra
    spectra_of_interest <- ms2_data_EXO_neg[ms2_data_EXO_neg$peak_id %in% rownames(peaks_of_interest)]
    ...
    ms2_spectra_EXO_neg[[i]] <- combined_spectra_of_interest
...
mgf_text <- NULL
for (i in names(ms2_spectra_EXO_neg)) {
